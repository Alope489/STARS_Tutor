...
class CodeBot(Chatbot):
    ...
    def delete_chat(chatbot, user_id):
      if st.session_state.selected_bot == "CodeBot":
        """Move the current chat from coderbot_chat_histories to deleted_chats."""
        ...
        if current_chat_id:
            ...
            chat_contents = user_doc.get("coderbot_chat_histories", {}).get(current_chat_id, {})
            
            # Move the chat contents to deleted_chats
            result = chatbot.users_collection.update_one(
                {"username": user_id},
                {
                    "$unset": {f"coderbot_chat_histories.{current_chat_id}": ""},  # Remove from coderbot_chat_histories
                    "$set": {
                        f"deleted_chats.{current_chat_id}": chat_contents,  # Add to deleted_chats
                        "current_chat_id_coderbot": None  # Reset the current chat ID
                    }
                }
            )
